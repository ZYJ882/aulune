...
Aulune 1.5.0 将服务商的 API Key、模型名称、接口基础地址和调用协议保存在当前设备的 Android Keystore 加密偏好中。用户可以为每个服务商分别保存配置，并可随时修改预设基础地址。普通对话、内容 AI 解析和画像候选会使用保存时选定的同一服务商配置。
...
在“模型”页选择服务商后，填写自己的 API Key。接口基础地址为普通可编辑字段；若服务商使用兼容网关，可将其修改为网关提供的 HTTPS 基础地址。然后选择调用协议、填写模型名，或点击“获取模型列表”后点选返回的模型。
...
端点输入应使用 HTTPS。对于 OpenAI 兼容、Anthropic 与 Gemini 自定义网关，使用者须根据其网关文档选择适当协议并填写正确的基础地址和模型名。

## 官方资料

以下链接用于确认默认服务的公开接口形状；模型可用性、账号权限和具体模型名称以服务商账户及其最新文档为准。

1. [Z.AI Quick Start](https://docs.z.ai/guides/overview/quick-start)：OpenAI 兼容的 Chat Completions 地址与 Bearer 鉴权。
2. [Kimi API Overview](https://platform.kimi.ai/docs/api/overview) 与 [Chat API](https://platform.kimi.ai/docs/api/chat)：OpenAI 兼容的 `/v1/chat/completions`、`/v1/models` 与响应结构。
3. [OpenRouter Quickstart](https://openrouter.ai/docs/quickstart)：`/api/v1/chat/completions` 和模型目录。
4. [Anthropic Models API](https://platform.claude.com/docs/en/api/models/list)：模型目录与 Messages API 相关接口。
5. [Gemini Models API](https://ai.google.dev/api/models)：模型目录与 GenerateContent 请求。
