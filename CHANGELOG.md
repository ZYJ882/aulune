...
v1.8.7 修复内容库空状态的“去首页”入口，使其能够直接跳转至“灵感”页；对话页的服务商状态条现可直接进入模型设置，未配置模型时会明确提示并提供同一路径。本机画像页在云端 AI 未配置时不再执行无效请求，而是引导用户前往设置；首页自省卡片的操作提示也已改为与实际点击行为一致的文案。

模型工作台现在要求先填写 API Key 才能启用云端调用。保存云端增强配置时，空 Key 不会再沿用上一服务商的 Key，从而避免切换服务商时发生错误调用。清除操作改为只删除当前服务商的本机加密配置，并增加二次确认；若该服务商正用于云端增强，会同时停止对应增强。新增回归测试覆盖空 Key 禁止启用、跨服务商不继承 Key 和有效配置归一化。

本次 GitHub `v1.8.7` 为**临时 Android Debug 证书签名的正式发布资产**。由于签名证书不同，它**不能覆盖安装**既有使用固定发布证书签名的版本（包括 v1.8.6）；安装前必须先卸载旧版本，卸载会清除旧版本的本机数据。后续若恢复固定发布证书，可重新发布支持覆盖升级的版本。
