...
多模型请求会从设备 HTTPS 直连选定服务商。API Key、模型名、基础地址与必要的协议配置使用 Android Keystore 加密保存在本机；预设服务商自动采用官方默认接口格式，协议选择仅对自定义服务商显示；模型列表获取失败时仍可手动填写模型名。信息流的“云端智能整理”只在用户点击后运行：单轮最多依次发送五条内容的标题、摘要、来源和当前规则主题，成功的分类与候选理由写回本机，失败内容继续保持本机规则。它不在后台运行，也不发送登录 Cookie、账户令牌、原始观看记录或设备标识。使用者只应填写自己拥有并获授权使用的 API Key，并自行承担相关服务商的数据处理、用量和计费责任。Aulune 不包含本地模型或向量嵌入功能，以避免额外的设备内存占用。详细接口适配和官方资料见 [多服务商接入说明](docs/model-providers.md)。
