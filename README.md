...
| 模块 | 当前实现 |
|---|---|
| Material You 界面系统 | 统一 8dp 网格、20dp 圆角、低对比卡片、浅边框、胶囊导航、轻触缩放与 Material Motion 过渡 |
| 外观模式与动态取色 | 设置页提供“跟随系统、浅色模式、深色模式”三种自动保存选项；切换立即生效，跟随系统会实时响应系统主题变化；Android 12+ 支持动态取色 |
...
外观偏好是非敏感的本机设置。选择“跟随系统”后，Aulune 会在系统主题变化时即时同步；选择浅色或深色后会立刻覆盖系统主题，不需要重启或确认。主题变更使用约 220ms 的颜色淡入淡出，并保持当前页面与滚动位置。Android 12 及以上设备优先采用系统动态颜色；其他设备使用 Aulune 的紫蓝协调色板。已保存的手动主题会在启动首帧同步系统栏，减少视觉闪屏。
