...
| 模块 | 当前实现 |
|---|---|
| 本地洞察工作台 | 本地主题内容卡、收藏/标记、偏好画像和关注主题展示 |
| 多模型文本对话 | 可配置 OpenAI、Claude、Gemini、DeepSeek 的 API Key 与模型名称；请求从设备通过 HTTPS 直连服务商 |
| 本地 Key 边界 | API Key 不写入源码、APK、日志或中转服务；当前进程退出后清除 |
| B 站官方网页会话 | 在受控 WebView 中访问官方登录页、账号中心、创作中心和消息页 |
| 四方式认证引导 UI | 扫码、密码、短信验证码与 Cookie 四个 Tab 的切换、说明和按钮交互 |
| 自定义品牌资源 | 独立应用名称、包名、主题与 Android 多密度启动器图标 |
| 版本发布 | 各版本 Debug APK 均保留在仓库 `releases/` 目录，并作为 GitHub Release 资产发布 |
| 自动构建 | 每次 PR 成功构建均自动发布公开预览 APK；主分支源码或受控源码压缩包更新自动构建，版本号变化时自动发布稳定 Release |
...
| OpenBiliClaw 的跨平台内容采集与自进化 Agent | 未实现 | 未接入 B 站、小红书、抖音、YouTube、X、知乎、Reddit、微博或开放 Web 的自动采集与任务编排 |
| 多账号会话、自动投稿、自动评论、消息抓取 | 未实现 | 不提供自动化账号操作或凭据导入能力 |
| 服务端、云同步、账户体系 | 未实现 | 当前版本不含 Aulune 自有后端，也不提供跨设备同步 |
| 生产签名与应用商店发布 | 未实现 | 当前为 Debug APK，仅用于本地学习和测试 |
...
Release 目录内的 APK 是 Debug 构建，使用本地调试签名，适合个人测试与学习。正式发布前应使用独立的 release keystore，并完成真实设备的隐私、安全、网络兼容性与服务商条款评审。
